...
async def test_smoke():
    ...
    assert 2 == len(result.data['users']), "There should be two users in the result set"
    ...
    assert 4 == len(result.data['createUser']), "The result set should have four fields"
    assert "john.doe@example.com" == result.data['createUser']['email'], "The email should match the input data"
    ...
    assert 4 == len(result.data['user']), "The result set should have four fields"
    ...
    assert 1 == len(result.data['user']), "A query with variables should return a single data value"
